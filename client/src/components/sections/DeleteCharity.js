...
const DeleteCharity = ({ id, modalHandler, update }) => {
  const charityDeleteHandler = async () => {
    try {
      await axios.delete(`http://103.107.184.159:5001/api/v1/charity/${id}`)
      toast.success('Charity Deleted!')
      update(true)
    } catch (error) {
      toast.error('Failed To Delete Chairty!')
      console.log(error)
    }
  }
